...
public class BiduStockService(HttpClient httpClient, IOptions<YieldCurveSourcesConfig> options, IServiceScopeFactory scopeFactory)
  : BackgroundService
{
  ...
  protected override async Task ExecuteAsync(CancellationToken cancellationToken)
  {
    ...
    while (!cancellationToken.IsCancellationRequested)
    {
      ...
      foreach (var line in lines)
      {
        ...
        {
          dbContext.BiduPrices.Add(new BiduStockPrice
          {
            Date = date,
            Open = open,
            High = high,
